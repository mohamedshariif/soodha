...
export async function createCategory(formData: FormData) {
  try {
    ...
    const color =
      rawColor &&
      CATEGORY_COLORS.includes(rawColor as (typeof CATEGORY_COLORS)[number])
        ? rawColor
        : CATEGORY_COLORS[0];
    ...
    if (exisiting) {
      throw new Error(
        `"${name}" already exisits in ${type === "INCOME" ? "Income" : "Expense"} categories.`,
      );
    }
  ...
  try {
    ...
    if (isUsed) {
      ...
      return actionSuccess(
        `"${category.name}" was archieved since it's in use.`,
      );
    }
