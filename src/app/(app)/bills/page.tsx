...
export default async function BillsPage() {
  ...
  const { periodStart, periodEnd } = parseMonthInputToBudgetPeriod(currentMonthValue);

  const [expenseCategories, bills, paidThisMonth] = await Promise.all([
    prisma.category.findMany({
      where: { userId: appUser.id, type: "EXPENSE", status: "ACTIVE", deletedAt: null },
    ...
    // One query, scoped by when it was actually PAID (not its original due date),
    // used for both the "Paid" summary card and the Paid tab.
  ...
  const remainingBillsThisMonth = bills.filter(
    (bill) => bill.nextDueDate.getTime() >= periodStart.getTime() && bill.nextDueDate.getTime() <= periodEnd.getTime()
  );
  ...
  const totalBillsThisMonthMinor = paidThisMonthMinor + remainingThisMonthMinor;

  const currency = bills[0]?.currency ?? paidThisMonth[0]?.currency ?? "USD";

  return (
    <div>
      <PageHeader title="Bills" description="Track upcoming bills before they become real expenses.">
        <AddBillModal expenseCategories={expenseCategoryOptions} today={todayInputValue} />
      </PageHeader>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          helper={`${remainingBillsThisMonth.length + paidThisMonth.length} bills`}
        ...
        <SummaryCard
          ...
          helper={`${paidThisMonth.length} payment${paidThisMonth.length === 1 ? "" : "s"} this month`}
        ...
        <SummaryCard
          label="Remaining"
          value={formatMoneyFromMinorUnits(remainingThisMonthMinor, currency)}
          helper={`${remainingBillsThisMonth.length} unpaid this month`}
          icon={<Clock className="h-5 w-5" />}
          valueClassName={remainingThisMonthMinor > BigInt(0) ? "text-amber-600" : "text-foreground"}
        ...
        <SummaryCard
          ...
          valueClassName={overdue.length > 0 ? "text-red-600" : "text-foreground"}
          iconClassName="bg-red-50 text-red-600"
        />
      </div>

      <div className="mt-6">
        <BillTabs upcomingBills={upcoming} overdueBills={overdue} paidPayments={paidThisMonth} today={todayDate} />
      </div>
    </div>
  );
}